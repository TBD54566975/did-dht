...
describe('did-dht', () => {
  ...
  describe('did publishing and resolving', function () {
    ...
    it('should publish and get a did document', async () => {
      const {did, keySet} = await DidDhtMethod.create();
      const didResolutionResult = await DidDhtMethod.publish(keySet, did);

      expect(didResolutionResult).to.exist;
      expect(didResolutionResult.didDocument).to.exist;
      expect(didResolutionResult.didDocument.id).to.equal(did.id);
      ...
      expect(didResolutionResult.didDocument.verificationMethod[0].id).to.equal(`${did.id}#0`);
      ...
      const gotDid = await DidDhtMethod.resolve(did.id);
