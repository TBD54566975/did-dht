...
const (
	advertisePeriod     = time.Minute * 5
	peerDiscoveryPeriod = time.Minute * 10
	peerLimit           = 10
...
func (s *Service) setupServiceIdentity() (*crypto.Ed25519PrivateKey, error) {
	gotDID, gotPrivKey, err := s.storage.ReadIdentity()
	if err != nil {
	...
	if gotDID != "" && gotPrivKey != nil {
		logrus.Infof("found existing identity: %s", gotDID)
		privKey = gotPrivKey
		k := key.DIDKey(gotDID)
		didKey = &k
	} else {
	...
	if err != nil {
		return nil, util.LoggingErrorMsg(err, "failed to expand gotDID key")
	}
